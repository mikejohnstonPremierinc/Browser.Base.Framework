...
namespace Mainpro.UITest
{
    ...
    [TestFixture]
    public class Mainpro_VoluntaryCycle_Tests : TestBase
    {
        #region Constructors
        public Mainpro_VoluntaryCycle_Tests(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }
        public Mainpro_VoluntaryCycle_Tests(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
        ...
        [Test]
        [Description("Verifying the following for a Voluntary user: 5 year cycle Timeframe, " +
            "No Credit Requirements, No AMA/RCP Link, No Annual CreditSummaryTable " +
            "and No Leave Adjustment ")]
        [Author("Bama Thangaraj")]
        public void VoluntaryCycle_PrimaryValidations()
        {
            ...
            Assert.False(DP.ClickHereToViewYourAmaRCPCreditsLnk.Displayed, "ClickHereToViewYourAmaRCPCreditsLink " +
                "should not displayed for voluntary cycle user");
            Assert.True(Help.GetCycleYearsTotal(Browser) == 5, "Voluntary user should be in 5 year cycle range");
            ...
            Assert.True(Browser.FindElement(By.XPath("//div[@class='error warning']")).Text.
                ...
                "Leave Adjustments should not be applied to Voluntary cycle user");

        }

        [Test]
        [Description("Given I adjust a user into a voluntary cycle, When i add Royal College MOC Accredited," +
        ...
        [Author("Bama Thangaraj")]
        public void VoluntaryCycle_AMARCPactivity_NoMaxLimit()
        {
            ...
            /// 2. Add an AMA activity with credits = 70.5 and RCP activity with credits = 70  
            Help.AddActivity(browser, TestContext.CurrentContext.Test, actCategory1, actCertType, actType1, actFormat,
                username: user.Username, creditsRequested: 70.5);
            ...
            /// 4. Verified the submitted credits in CYCLE year Table 
