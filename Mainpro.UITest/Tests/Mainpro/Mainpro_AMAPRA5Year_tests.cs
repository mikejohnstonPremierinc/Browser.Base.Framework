...
namespace Mainpro.UITest
{
    ...
    [TestFixture]
    public class Mainpro_AMAPRA5Year_tests : TestBase
    {
        #region Constructors
        public Mainpro_AMAPRA5Year_tests(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }

        // Remote Selenium Grid Test
        public Mainpro_AMAPRA5Year_tests(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
        ...
        [Test]
        [Description("Given a user with a Default cycle tries to submit an AMA activity with 200 credits, When the " +
            "user clicks the Submit button, Then the user should be prompted of the Max Credits rules outlining " +
        ...
        [Category("Complete")]
        public void AMAPRAAdd200CreditsCertified400NonCertified()
        {
            ...
            /// 1. For a user in a Default cycle, enter 200 credits for an AMA PRA Group Learning activity and 
            ...
            /// 4. Enter 400 in the Credits field, submit the activity, then verify that the CPD Activity List 
            ...
            Help.VerifyCellTextMatches(browser, ALP, Const_Mainpro.Table.AMARCPMaxCreditForm,
                rowName: "AMA PRA Category 1", colIndex: "2", cellTextExpected: "200");
            Help.VerifyCellTextMatches(browser, ALP, Const_Mainpro.Table.AMARCPMaxCreditForm,
                rowName: "AMA PRA Category 1", colIndex: "3", cellTextExpected: "50");
        }

        // Fixed defect https://code.premierinc.com/issues/browse/MAINPROREW-841. Uncomment and execute when fixed
        // Outstanding defect https://code.premierinc.com/issues/browse/MAINPROREW-847. See comments inside the test for 
        // more info. Uncomment and execute when fixed
        [Test]
        [Description("Given a user with a Default cycle submits an AMA PRA activity with 30 credits and another one with " +
            "30 credits, When the second one is submitted, Then the user should be prompted of the Max Credits " +
            "rules outlining how many credits will be applied and not applied, and the Activities List page " +
            "should function accordingly in terms of reporting and applying credit appropriately, and When the user " +
            "tries to add another AMA PRA activity and selects the Activity Type in the Select Element on the Enter " +
            "an Activity page, Then the user should be prompted of the Max Credits rules and the buttons on the " +
            "prompt should function accordingly")]
        [Property("Status", "Complete")]
        [Author("Mike Johnston")]
        public void AMAPRAAdd2CertifiedActiviesOverMax()
        {
            // Store the test data into variables
            var actCategory1 = Const_Mainpro.ActivityCategory.GroupLearning;
            var actType1 = Const_Mainpro.ActivityType.GRPLRNING_CERT_AmericanMedicalAssociationAMAPRACategory1_LO;
            var actCategory2 = Const_Mainpro.ActivityCategory.SelfLearning;
            var actType2 = Const_Mainpro.ActivityType.SELFLRNING_CERT_AmericanMedicalAssociationAMAPRACategory1_LO;
            var actCertType = Const_Mainpro.ActivityCertType.Certified;
            var actFormat = Const_Mainpro.ActivityFormat.Live;
            ...
            /// 2. Enter 30 credits for an AMA PRA Self Learning activity and click the Submit button then 
