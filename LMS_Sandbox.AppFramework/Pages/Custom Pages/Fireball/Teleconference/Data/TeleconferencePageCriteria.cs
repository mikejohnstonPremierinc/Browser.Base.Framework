...
namespace LMS.AppFramework
{
    ...
    /// </summary>
    public class TeleconferencePageCriteria
    {
        /// <summary>
        ...
        /// </summary>
        public readonly ICriteria<TeleconferencePage> CalendarTblFirstRowVisible = new Criteria<TeleconferencePage>(p =>
        {
            return p.Exists(Bys.TeleconferencePage.CalendarTblFirstRow, ElementCriteria.IsVisible);

        }, "The first row from the Calendar table table exists and is visible");

        public readonly ICriteria<TeleconferencePage> SearchBtnVisible = new Criteria<TeleconferencePage>(p =>
        {
            return p.Exists(Bys.LMSPage.SearchTxt, ElementCriteria.IsVisible);

        }, "Search button exists and is visible");

        public readonly ICriteria<TeleconferencePage> Mobile_SearchMagnifyingGlassBtnVisible = new Criteria<TeleconferencePage>(p =>
        {
        ...
        public readonly ICriteria<TeleconferencePage> LoadIconNotExists = new Criteria<TeleconferencePage>(p =>
        {
            return !p.Exists(Bys.LMSPage.LoadIcon);

        }, "Load icon not exists");

        public readonly ICriteria<TeleconferencePage> CustomPageHTMLComponentNonGroupsVisible = new Criteria<TeleconferencePage>(p =>
        {
        ...
        /// page to be loaded. Note below that I am using the AND condition to combine the 3 criteria above. <see cref="TeleconferencePage.WaitForInitialize"/>
        ...
        /// </summary>
        public readonly ICriteria<TeleconferencePage> PageReady;

        public TeleconferencePageCriteria()
        {
