...
namespace LMS.AppFramework
{
    ...
    /// </summary>
    public class CERequestPageCriteria
    {
        /// <summary>
        ...
        /// </summary>
        public readonly ICriteria<CERequestPage> CERequestsHeaderLblVisible = new Criteria<CERequestPage>(p =>
        {
            return p.Exists(Bys.CERequestPage.CERequestsHeaderLbl, ElementCriteria.IsVisible);

        }, "CE Requests header label visible");

        public readonly ICriteria<CERequestPage> VideographyLnkExists = new Criteria<CERequestPage>(p =>
        {
            return p.Exists(Bys.CERequestPage.VideographyLnk);

        }, "Videography link exists");

        public readonly ICriteria<CERequestPage> SearchBtnVisible = new Criteria<CERequestPage>(p =>
        {
            return p.Exists(Bys.LMSPage.SearchTxt, ElementCriteria.IsVisible);

        }, "Search button exists and is visible");

        public readonly ICriteria<CERequestPage> Mobile_SearchMagnifyingGlassBtnVisible = new Criteria<CERequestPage>(p =>
        {
        ...
        public readonly ICriteria<CERequestPage> LoadIconNotExists = new Criteria<CERequestPage>(p =>
        {
            return !p.Exists(Bys.LMSPage.LoadIcon);

        }, "Load icon not exists");

        public readonly ICriteria<CERequestPage> CustomPageHTMLComponentNonGroupsVisible = new Criteria<CERequestPage>(p =>
        {
            return p.Exists(Bys.LMSPage.CustomPageHTMLComponentNonGroups, ElementCriteria.IsVisible);

        }, "The non-group HTML component exists and is visible");
        ...
        /// page to be loaded. Note below that I am using the AND condition to combine the 3 criteria above. <see cref="CERequestPage.WaitForInitialize"/>
        ...
        /// </summary>
        public readonly ICriteria<CERequestPage> PageReady;

        public CERequestPageCriteria()
        {
            PageReady = CustomPageHTMLComponentNonGroupsVisible.AND(LoadIconNotExists);
        }
    }
}
