...
namespace LMS.AppFramework
{
    ...
    /// </summary>
    public class GenericCustomPageCriteria
    {
        /// <summary>
        ...
        /// </summary>
        public readonly ICriteria<GenericCustomPage> CustomPageHTMLComponentGroupsVisible = new Criteria<GenericCustomPage>(p =>
        {
            return p.Exists(Bys.LMSPage.CustomPageHTMLComponentGroups, ElementCriteria.IsVisible);

        }, "HTML Component Group exists and is visible");

        public readonly ICriteria<GenericCustomPage> CustomPageHTMLComponentNonGroupsVisible = new Criteria<GenericCustomPage>(p =>
        {
            return p.Exists(Bys.LMSPage.CustomPageHTMLComponentNonGroups, ElementCriteria.IsVisible);

        }, "HTML Component Non-Group exists and is visible");

        public readonly ICriteria<GenericCustomPage> SearchBtnVisible = new Criteria<GenericCustomPage>(p =>
        {
            return p.Exists(Bys.LMSPage.SearchTxt, ElementCriteria.IsVisible);

        }, "Search button exists and is visible");

        public readonly ICriteria<GenericCustomPage> Mobile_SearchMagnifyingGlassBtnVisible = new Criteria<GenericCustomPage>(p =>
        {
        ...
        public readonly ICriteria<GenericCustomPage> LoadIconNotExists = new Criteria<GenericCustomPage>(p =>
        {
        ...
        /// page to be loaded. Note below that I am using the AND condition to combine the 3 criteria above. <see cref="GenericCustomPage.WaitForInitialize"/>
        ...
        /// </summary>
        public readonly ICriteria<GenericCustomPage> PageReady;

        public GenericCustomPageCriteria()
        {
            PageReady = CustomPageHTMLComponentGroupsVisible.AND(LoadIconNotExists)
                .OR(CustomPageHTMLComponentNonGroupsVisible).AND(LoadIconNotExists);
