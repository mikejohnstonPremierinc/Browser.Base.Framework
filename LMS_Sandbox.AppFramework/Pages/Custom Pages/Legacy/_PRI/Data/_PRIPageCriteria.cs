...
namespace LMS.AppFramework
{
    ...
    public class _PRIPageCriteria
    {
        ...
        /// </summary>
        public readonly ICriteria<_PRIPage> ProfessionalEducationTblFirstLnkVisible = new Criteria<_PRIPage>(p =>
        {
            return p.Exists(Bys._PRIPage.ProfessionalEducationTblFirstLnk, ElementCriteria.IsVisible);

        }, "The first link from the Professional Education table exists and is visible");

        public readonly ICriteria<_PRIPage> MyRecentActivitiesTblFirstLnkVisible = new Criteria<_PRIPage>(p =>
        {
            return p.Exists(Bys._PRIPage.MyRecentActivitiesTblFirstLnk, ElementCriteria.IsVisible);

        }, "The first link from the My Recent Activities table exists and is visible");

        public readonly ICriteria<_PRIPage> FeaturedActivitiesTblFirstLnkVisible = new Criteria<_PRIPage>(p =>
        {
            return p.Exists(Bys._PRIPage.FeaturedActivitiesTblFirstLnk, ElementCriteria.IsVisible);

        }, "The first link from the Featured Activities table exists and is visible");

        public readonly ICriteria<_PRIPage> SearchBtnVisible = new Criteria<_PRIPage>(p =>
        {
            return p.Exists(Bys.LMSPage.SearchTxt, ElementCriteria.IsVisible);

        }, "Search button exists and is visible");

        public readonly ICriteria<_PRIPage> Mobile_SearchMagnifyingGlassBtnVisible = new Criteria<_PRIPage>(p =>
        {
        ...
        public readonly ICriteria<_PRIPage> LoadIconNotExists = new Criteria<_PRIPage>(p =>
        {
        ...
        /// page to be loaded. Note below that I am using the AND condition to combine the 3 criteria above. <see cref="_PRIPage.WaitForInitialize"/>
        ...
        /// </summary>
        public readonly ICriteria<_PRIPage> PageReady;

        public _PRIPageCriteria()
        {
