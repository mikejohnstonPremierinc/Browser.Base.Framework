...
namespace LMS.AppFramework
{
    ...
    public class _CAPPageCriteria
    {
        ...
        /// </summary>
        public readonly ICriteria<_CAPPage> UpcomingCapPresTblFirstLnkVisible = new Criteria<_CAPPage>(p =>
        {
            return p.Exists(Bys._CAPPage.UpcomingCapPresTblFirstLnk, ElementCriteria.IsVisible);

        }, "The first link from the Upcoming CAP Presentation table exists and is visible");

        public readonly ICriteria<_CAPPage> RegSchedTeleconfTblFirstLnkVisible = new Criteria<_CAPPage>(p =>
        {
            return p.Exists(Bys._CAPPage.RegSchedTeleconfTblFirstLnk, ElementCriteria.IsVisible);

        }, "The first link from the Regularly Scheduled Teleconference table exists and is visible");

        public readonly ICriteria<_CAPPage> ProfessionalEduTblFirstLnkVisible = new Criteria<_CAPPage>(p =>
        {
            return p.Exists(Bys._CAPPage.ProfessionalEduTblFirstLnk, ElementCriteria.IsVisible);

        }, "The first link from the Professional Education table exists and is visible");

        public readonly ICriteria<_CAPPage> SearchBtnVisible = new Criteria<_CAPPage>(p =>
        {
            return p.Exists(Bys.LMSPage.SearchTxt, ElementCriteria.IsVisible);

        }, "Search button exists and is visible");

        public readonly ICriteria<_CAPPage> Mobile_SearchMagnifyingGlassBtnVisible = new Criteria<_CAPPage>(p =>
        {
        ...
        public readonly ICriteria<_CAPPage> LoadIconNotExists = new Criteria<_CAPPage>(p =>
        {
        ...
        /// page to be loaded. Note below that I am using the AND condition to combine the 3 criteria above. <see cref="_CAPPage.WaitForInitialize"/>
        ...
        /// </summary>
        public readonly ICriteria<_CAPPage> PageReady;

        public _CAPPageCriteria()
        {
            PageReady = UpcomingCapPresTblFirstLnkVisible.AND(RegSchedTeleconfTblFirstLnkVisible).
                AND(ProfessionalEduTblFirstLnkVisible).AND(LoadIconNotExists);
        }
    }
}
