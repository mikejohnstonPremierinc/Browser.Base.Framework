...
namespace Wikipedia.AppFramework
{
    ...
    public class HomePageCriteria
    {
        ...
        /// </summary>                       
        public readonly ICriteria<HomePage> WikipediaLnkVisible = new Criteria<HomePage>(p =>
        {
            return p.Exists(Bys.HomePage.WikipediaLnk, ElementCriteria.IsVisible);

        }, "Wikipedia link exists and is visible");


        public readonly ICriteria<HomePage> SearchTxtVisible = new Criteria<HomePage>(p =>
        {
            return p.Exists(Bys.WikipediaBasePage.SearchTxt, ElementCriteria.IsVisible);

        }, "Search text box exists and is visible");

        public readonly ICriteria<HomePage> WikipediaImgVisible = new Criteria<HomePage>(p =>
        {
            return p.Exists(Bys.HomePage.WikipediaImg, ElementCriteria.IsVisible);
        ...
        {
            PageReady = WikipediaLnkVisible.AND(SearchTxtVisible).OR(WikipediaImgVisible);
        }
    }
}
