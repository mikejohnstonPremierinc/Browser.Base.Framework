...
namespace CMECA.UITest
{
    ...
    /// </summary>
    [TestFixture]
    public class CMECA_Registration_AllBrowsers : TestBase_CMECA
    {
        #region Constructors

        public CMECA_Registration_AllBrowsers(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }
        public CMECA_Registration_AllBrowsers(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
    ...
    [TestFixture]
    public class CMECA_Registration_ChromeSet_1 : TestBase_CMECA
    {
        #region Constructors

        public CMECA_Registration_ChromeSet_1(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }
        public CMECA_Registration_ChromeSet_1(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
                                    : base(browserName, emulationDevice, version, platform, hubUri, extrasUri)
        { }
    ...
    "application should allow me to advance")]
        ...
        [TestCase(siteCodeAttribute), Category(siteCodeCategory), Category("QA"), Category("UAT"), Category("Prod")]
        [Description("Assert the the Community eCommerce page loads properly")]
        [Property("Status", "Complete")]
        [Author("Mike Johnston")]
        public void Reg_Payment(Constants.SiteCodes siteCode)
        {
            string activityTitle = Constants.ActTitle.Automation_Registration_10PaymentNeeded2_CMECA.GetDescription();
            UserModel user = profession2User1;
            APIHelp.DeleteActivityForUser(user.Username, activityTitle, siteCode);

            /// 1. Log in, go to the Preview page, click Register and assert that the Community payment page appears
            ActPreviewPage APP = Help.GoTo_ActivityNonWorkflow_PreviewPageViaURL(Browser, siteCode, activityTitle,
                false, user.Username);
            APP.RegisterBtn.Click();
            browser.WaitJSAndJQuery(TimeSpan.FromSeconds(60));
            Assert.True(Browser.Exists(By.XPath("//input[contains(@name, 'txtFirstName')]"), ElementCriteria.IsVisible), 
                "The payment page did not load properly");
        }
    ...
    [TestFixture]
    public class CMECA_Registration_ChromeSet_2 : TestBase_CMECA
    {
        #region Constructors

        public CMECA_Registration_ChromeSet_2(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }
        public CMECA_Registration_ChromeSet_2(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
                                    : base(browserName, emulationDevice, version, platform, hubUri, extrasUri)
        { }
