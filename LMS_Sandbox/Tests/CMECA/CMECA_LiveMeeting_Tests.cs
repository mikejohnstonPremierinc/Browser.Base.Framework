...
namespace CMECA.UITest
{
    ...
    [TestFixture]
    public class CMECA_LiveMeeting_Tests : TestBase_CMECA
    {
        #region Constructors

        public CMECA_LiveMeeting_Tests(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }
        public CMECA_LiveMeeting_Tests(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
        ...
        public void StandAloneLiveInfoOnTranscript(Constants.SiteCodes siteCode)
        {
            ...
            List<Constants.Transcript> TranscriptForMyActivity = 
                TP.GetTranscript(Constants.SiteCodes.CMECA).Where(t => t.ActivityTitle == actTitle).ToList();
