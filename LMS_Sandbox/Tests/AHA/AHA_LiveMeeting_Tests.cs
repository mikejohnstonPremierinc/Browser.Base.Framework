...
namespace AHA.UITest
{
    ...
    [TestFixture]
    public class AHA_LiveMeeting_Tests : TestBase_AHA
    {
        #region Constructors

        public AHA_LiveMeeting_Tests(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }
        public AHA_LiveMeeting_Tests(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
        ...
        public void StandAloneLiveInfoOnTranscript(Constants.SiteCodes siteCode)
        {
            ...
            List<Constants.Transcript> TranscriptForMyActivity = 
                TP.GetTranscript(Constants.SiteCodes.AHA).Where(t => t.ActivityTitle == actTitle).ToList();
