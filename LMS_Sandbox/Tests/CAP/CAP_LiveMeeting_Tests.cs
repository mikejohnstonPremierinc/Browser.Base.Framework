...
namespace CAP.UITest
{
    ...
    [TestFixture]
    public class CAP_LiveMeeting : TestBase_CAP
    {
        #region Constructors

        public CAP_LiveMeeting(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }
        public CAP_LiveMeeting(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
        ...
        public void StandAloneLiveInfoOnTranscript(Constants.SiteCodes siteCode)
        {
            ...
            List<Constants.Transcript> TranscriptForMyActivity = 
                TP.GetTranscript(Constants.SiteCodes.CAP).Where(t => t.ActivityTitle == actTitle).ToList();
