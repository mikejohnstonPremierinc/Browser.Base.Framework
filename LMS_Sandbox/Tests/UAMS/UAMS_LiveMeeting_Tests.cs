...
namespace UAMS.UITest
{
    ...
    [TestFixture]
    public class UAMS_LiveMeeting : TestBase_UAMS
    {
        #region Constructors

        public UAMS_LiveMeeting(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }
        public UAMS_LiveMeeting(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
        ...
        public void StandAloneLiveInfoOnTranscript(Constants.SiteCodes siteCode)
        {
            ...
            List<Constants.Transcript> TranscriptForMyActivity = 
                TP.GetTranscript(Constants.SiteCodes.UAMS).Where(t => t.ActivityTitle == actTitle).ToList();
