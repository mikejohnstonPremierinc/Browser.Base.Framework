...
namespace UAMS.UITest
{
    ...
    /// </summary>
    [TestFixture]
    public class UAMS_Registration_AllBrowsers : TestBase_UAMS
    {
        #region Constructors

        public UAMS_Registration_AllBrowsers(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }
        public UAMS_Registration_AllBrowsers(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
    ...
    [TestFixture]
    public class UAMS_Registration_ChromeSet_1 : TestBase_UAMS
    {
        #region Constructors

        public UAMS_Registration_ChromeSet_1(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }
        public UAMS_Registration_ChromeSet_1(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
                                    : base(browserName, emulationDevice, version, platform, hubUri, extrasUri)
        { }
      ...
      "button should appear")]
        ...
        [Description("Asserting the following registration scenarios are taking proper precedence in terms of order of appearance " +
            "1. Access Code " +
            "2. Registration page " +
            "3. CPE Monitor " +
            "4. Payment " +
        ...
        [Property("Status", "Complete")]
        [Author("Mike Johnston")]
        public void Reg_AcsCode_RegPage_CPE_Payment_OnHold(Constants.SiteCodes siteCode)
        {
            string activityTitle = Constants.ActTitle.Automation_Registration_7AccessCode_8RegistrationPage_9CPEMonitor_10Payment_11OnHold.GetDescription();
            UserModel user = profession2User1;
            APIHelp.DeleteActivityForUser(user.Username, activityTitle, siteCode);

            /// 1. Log in as a Pharmacist, go to the activity, click Register and assert that the Access Code modal appears
            ActPreviewPage APP = Help.GoTo_ActivityNonWorkflow_PreviewPageViaURL(Browser, siteCode, activityTitle, 
                false, user.Username);

            /// 2. Submit a valid Access Code and then assert that the Registraion page appears
            ActRegistrationPage ARP = APP.SubmitAccessCode(DBUtils.GetAccessCode(activityTitle));

            // Fixed Defect https://code.premierinc.com/issues/browse/LMSREW-1021
            /// 3.Enter required fields, submit and assert that the CPE Monitor form appears
            ActCPEMonitorPage CPEP = ARP.RegisterForActivity(user.Username);

            /// 4.Submit the CPE Monitor form and then assert that the Payment page appears
            // Fixed Defect https://code.premierinc.com/issues/browse/LMSREW-956
            ActOrderDetailsPage AODP = CPEP.FillAndSubmitForm();

            // Ignoring payment tests in UAMS. Per the team, UAMS does not have payment setup. Testing payment in 
            // CMECA instead
            /// 5. Enter a full discount code, submit, acknowledge the receipt page, and then assert that the On Hold page appears
            //// Fixed defect: https://code.premierinc.com/issues/browse/LMSREW-1940
            //if (Help.EnvironmentEquals(Constants.Environments.Production))
            //{
            //    Assert.Pass("Ignoring this in Prod. See defect https://code.premierinc.com/issues/browse/LMSREW-1940 " +
            //        "and Aruns comments. Originally thought this was a defect, but due to things with vanity URLs in " +
            //        "prod, etc., it is ok that the user is logged out in Prod");
            //}
            //ActOrderReceiptPage AORP = AODP.SubmitDiscountCode("full");
            //ActOnHoldPage AOP = AORP.ClickAndWait(AORP.ExitBtn);
        }
    ...
    "4. On Hold Expired")]
        [Property("Status", "Complete")]
        [Author("Mike Johnston")]
        public void Reg_AcsCode_RegPage_Payment_OnHoldExpired(Constants.SiteCodes siteCode)
        {
            // Ignoring test on Prod until next SP when hopefully payment is fixed. Rigth now it errors
            Help.IgnoreTestOnEnvironmentUntilDate(DateTime.ParseExact("06/18/2019", "MM/dd/yyyy", CultureInfo.InvariantCulture),
                Constants.Environments.Production);

            string activityTitle = Constants.ActTitle.Automation_Registration_7AccessCode_8RegistrationPage_10Payment_111OnHoldExpired.GetDescription();
            UserModel user = profession1User1;
            APIHelp.DeleteActivityForUser(user.Username, activityTitle, siteCode);

            /// 1. Log in, go to the activity, click Register and assert that the Access Code modal appears
            ActPreviewPage APP = Help.GoTo_ActivityNonWorkflow_PreviewPageViaURL(Browser, siteCode, activityTitle, false, user.Username);

            /// 2. Submit a valid Access Code and then assert that the Registraion page appears
            var blah = DBUtils.GetAccessCode(activityTitle);
            ActRegistrationPage ARP = APP.SubmitAccessCode(DBUtils.GetAccessCode(activityTitle));

            /// 3. Enter required fields, submit and assert that the Payment page appears
            ActOrderDetailsPage AODP = ARP.RegisterForActivity(user.Username);

            // Ignoring payment tests in UAMS. Per the team, UAMS does not have payment setup. Testing payment in 
            // CMECA instead
            ///// 4. Enter a full discount code, submit, acknowledge the receipt page, and then assert that the activity workflow starts
            //if (Help.EnvironmentEquals(Constants.Environments.Production))
            //{
            //    if (Help.EnvironmentEquals(Constants.Environments.Production))
            //    {
            ...
            //        //throw new Exception("Payment is expected to fail on Production. Per Arun: 'for the payment to work " +
            //        //"on production, the client has to cofigure theirpayment gateway account details'. Until then, we will " +
            //        //"continue to fail this test because it requires that payments are configured and working. Every SP, check " +
            //        //"with DEV to see if client has conigured details yet. If they have, then remove this If statement and run the test");
            //    }
            //}
            //ActOrderReceiptPage AORP = AODP.SubmitDiscountCode("full");
            //ActOverviewPage AOP = AORP.ClickAndWait(AORP.ExitBtn);
        }
    ...
    [TestFixture]
    public class UAMS_Registration_ChromeSet_2 : TestBase_UAMS
    {
        #region Constructors

        public UAMS_Registration_ChromeSet_2(string browserName, string emulationDevice) : base(browserName, emulationDevice) { }
        public UAMS_Registration_ChromeSet_2(string browserName, string emulationDevice, string version, string platform, string hubUri, string extrasUri)
                                    : base(browserName, emulationDevice, version, platform, hubUri, extrasUri)
        { }
        ...
        public void Reg_Payment_eCommerce_External(Constants.SiteCodes siteCode)
        {
            ...
            // 6/2/20: On Prod, clicking this button produced an App error. I was told UAMS eCommerce is not setup for 
            // clients, so we dont need to do this verification
